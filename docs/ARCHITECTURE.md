...
Current presentation: per-source extraction remains cached internally, then synthesis returns one cited answer per guide question. Expert views, agreements and substantive differences are combined without separate repeated result sections. Sources expand on demand. Completed history remains stored but is not automatically opened or exposed through a saved-run selector.
